...
const OBUS_USER_DEACTIVATE_GROUP_CONCURRENCY =
  Number.parseInt(
    ...
    10
  ) || Math.max(1, Number(OBUS_USER_DEACTIVATE_CONCURRENCY || 8));
const OBUS_USER_DEACTIVATE_DELETE_CONCURRENCY =
  Number.parseInt(
    process.env.OBUS_USER_DEACTIVATE_DELETE_CONCURRENCY || String(OBUS_USER_DEACTIVATE_CONCURRENCY || 8),
    10
  ) || Math.max(1, Number(OBUS_USER_DEACTIVATE_CONCURRENCY || 8));
...
function extractObusUsersWithoutPermissionsRows(
  ...
  const buildDebugSample = ({
    ...
    debug.samples.push({
      ...
      activeRaw: normalizeObusDebugPayload(activeRaw),
      statusKeywordRaw: normalizeObusDebugPayload(statusKeywordRaw),
      statusRaw: normalizeObusDebugPayload(statusRaw)
    });
  };
...
async function fetchObusUsersWithoutPermissionsForTarget({
  ...
  if (!endpointUrl) {
    debugItem.error = `${cluster}: GetUsersWithoutPermissions URL oluşturulamadı. (${companyRef})`;
    return {
      cluster,
      rows: [],
  ...
  if (!partnerCode) {
    debugItem.error = `${cluster}: partner-code boş olduğu için istek atlanıyor. (${companyRef})`;
    return {
      cluster,
      rows: [],
      error: debugItem.error,
  ...
  if (!sessionId || !deviceId || !token) {
    debugItem.error = `${cluster}: UserLogin sonrası session/device/token eksik.`;
    return {
      cluster,
      rows: [],
      error: debugItem.error,
...
function parseObusUserDeactivateValue(rawValue) {
  const text = String(rawValue || "").trim();
  if (!text) return null;
  const parts = text.split("|||");
  if (parts.length < 4) return null;
  const source = extractClusterLabel(parts[0]);
  const partnerId = String(parts[1] || "").trim();
  const id = String(parts[2] || "").trim();
  const username = String(parts.slice(3).join("|||") || "").trim();
  if (!source || !partnerId || !id || !username) return null;
  return {
    value: `${source}|||${partnerId}|||${id}|||${username}`,
    source,
    "partner-id": partnerId,
...
function normalizeObusUserDeactivateSelectedItem(rawItem) {
  const item = rawItem && typeof rawItem === "object" ? rawItem : {};
  const parsedFromValue = parseObusUserDeactivateValue(item.value);
  const source = extractClusterLabel(String(item.source || parsedFromValue?.source || "").trim());
  const partnerId = String(item["partner-id"] || item.partnerId || parsedFromValue?.["partner-id"] || "").trim();
  const id = String(item.id || parsedFromValue?.id || "").trim();
  const username = String(item.username || parsedFromValue?.username || "").trim();
  const code = String(item.code || "").trim();
  if (!source || !partnerId || !id || !username || !code) return null;
  return {
    value: `${source}|||${partnerId}|||${id}|||${username}`,
    source,
    "partner-id": partnerId,
...
async function deactivateObusUsersBySelection({ selectedItems, cacheRows, onItemResult }) {
  ...
  items.forEach((item) => {
    ...
    if (!cluster || !partnerCode) {
      ...
      failureItems.push(failureRecord);
      if (notifyItemResult) {
        notifyItemResult({
  ...
  const groupedEntries = Array.from(groupedByContext.entries());
  const groupedResults = await runWithConcurrency(
    groupedEntries,
    Math.max(1, OBUS_USER_DEACTIVATE_GROUP_CONCURRENCY),
    async ([contextKey, contextGroup]) => {
      const localSuccessItems = [];
      const localFailureItems = [];
      const pushLocalSuccess = (item, message) => {
        const record = {
          label: buildObusUserDeactivateItemLabel(item),
          message: String(message || "Kullanıcı pasife alındı.").trim()
        };
  ...
  groupedResults.forEach((groupResult) => {
    if (!groupResult || typeof groupResult !== "object") return;
    (Array.isArray(groupResult.successItems) ? groupResult.successItems : []).forEach((item) => {
      successItems.push(item);
    });
    (Array.isArray(groupResult.failureItems) ? groupResult.failureItems : []).forEach((item) => {
...
async function loadObusUserCreatePageState({ selectedCompaniesInput, formSource }) {
  ...
  const [cacheResult, obusMerkezMapResult] = await Promise.all([
    fetchAllCompaniesRowsFromCache(),
    fetchAllCompaniesObusMerkezSubeIdMap()
  ]);
  const cacheRows = Array.isArray(cacheResult?.rows) ? cacheResult.rows : [];
  const partnerItems = normalizePartnerItems(
    cacheRows.map((row) => ({
  ...
  const { map: obusMerkezSubeIdByCompany, error: obusCacheError } = obusMerkezMapResult;
...
async function createObusUsersByCompanies({ companies, formValues, onItemResult }) {
  const selectedCompanies = Array.isArray(companies) ? companies : [];
  const safeFormValues = normalizeObusUserCreateFormValues(formValues);
  const successItems = [];
  const failureItems = [];
  const notifyItemResult =
    typeof onItemResult === "function"
  ...
  const workerResults = await runWithConcurrency(
    selectedCompanies,
    Math.max(1, OBUS_USER_CREATE_CONCURRENCY),
    async (company) => {
      const companyLabel = String(company?.label || "").trim() || String(company?.value || "").trim() || "Firma";
      let result;
      try {
        result = await createObusUserForCompany({ company, formValues: safeFormValues });
      } catch (err) {
        result = {
          ok: false,
          label: companyLabel,
  ...
  workerResults.forEach((payload, index) => {
    const company = selectedCompanies[index];
    const fallbackLabel = String(company?.label || "").trim() || String(company?.value || "").trim() || "Firma";
...
function normalizeObusBulkCreateUserEntries(rawEntries) {
  ...
  list.forEach((rawItem, index) => {
    ...
    let entryId = String(item.entryId || item.id || `row-${index + 1}`).trim() || `row-${index + 1}`;
...
function buildObusGetUsersWithoutPermissionsTargetForCompany(company) {
  ...
  const cluster = extractClusterLabel(String(meta.cluster || "").trim());
  const partnerCode = String(meta.code || "").trim();
  const partnerId = String(meta.id || "").trim();
  const companyValue = String(source.value || "").trim();
  const companyLabel = String(source.label || companyValue || "Firma").trim();
  const loginBranchId = String(source.obusMerkezSubeId || "").trim();
  const clusterSourceUrl = resolvePartnerFetchUrlByCluster(cluster);
  const endpointUrl = normalizeTargetUrl(
    buildMembershipGetUsersWithoutPermissionsUrl(clusterSourceUrl || OBUS_USER_DEACTIVATE_LIST_API_URL, cluster)
  );
  ...
  return {
    ...
    companyValue,
    companyLabel,
    obusMerkezSubeId: loginBranchId
  };
}
...
function normalizeObusBulkCreateTargetItems(rawTargets) {
  ...
  list.forEach((rawItem, index) => {
    ...
    const username = String(item.username || "").trim();
    const password = String(item.password || "");
    if (!companyValue && !fullName && !username && !password) return;

    const entryId = String(item.entryId || `row-${index + 1}`).trim() || `row-${index + 1}`;
    let key = String(item.key || `${companyValue}|||${entryId}`).trim();
    if (!key) key = `${companyValue}|||${entryId}`;
    if (usedKeys.has(key)) {
      key = `${key}-${index + 1}`;
    }
    usedKeys.add(key);
...
async function createObusUsersByTargetItems({ targets, companyByValue, onItemResult }) {
  const list = Array.isArray(targets) ? targets : [];
  const companyMap = companyByValue instanceof Map ? companyByValue : new Map();
  const successItems = [];
  const failureItems = [];
  const notifyItemResult =
    typeof onItemResult === "function"
  ...
  const workerResults = await runWithConcurrency(
    ...
    async (targetItem, index) => {
      const companyValue = String(targetItem?.companyValue || "").trim();
      const company = companyMap.get(companyValue);
      const username = String(targetItem?.username || "").trim();
      const fullName = String(targetItem?.fullName || "").trim();
      const password = String(targetItem?.password || "");
      const key = String(targetItem?.key || `${companyValue}|||${index + 1}`).trim() || `${companyValue}|||${index + 1}`;
      const companyLabel =
        String(company?.label || companyValue || "Firma").trim() || "Firma";
      ...
      let result;
      try {
        result = await createObusUserForCompany({
          company,
          formValues: {
            fullName,
            username,
      ...
      const isOk = normalizedResult.ok === true;
  ...
  try {
    ...
    const selectedCompaniesInput =
      normalizedBulkTargets.length > 0
        ...
        : parseSelectedCompanyValuesFromInput(req.body?.selectedCompanies);
    const formSource = {
    ...
    if (bulkMode === true && normalizedBulkTargets.length > 0) {
      const companyByValue = new Map(selectedCompanies.map((item) => [String(item.value || "").trim(), item]));
      const itemMeta = normalizedBulkTargets.map((item) => {
        const company = companyByValue.get(String(item.companyValue || "").trim());
        const companyLabel = String(company?.label || item.companyValue || "Firma").trim();
        return {
          key: String(item.key || "").trim(),
          label: `${companyLabel} | ${String(item.username || "").trim() || "Kullanıcı?"}`
        };
      });
      ...
      const job = createObusLiveJob({
        ...
        totalCount: normalizedBulkTargets.length
      });
      res.json({
        ok: true,
        jobId: job.id,
      ...
      setImmediate(async () => {
        try {
          await createObusUsersByTargetItems({
            targets: normalizedBulkTargets,
            companyByValue,
  ...
  async (req, res) => {
    try {
      ...
      return res.json({
        ...
        items: Array.isArray(checkResult.items) ? checkResult.items : []
      });
    } catch (err) {
      return res.status(500).json({
        ok: false,
...
app.post(
  "/api/obus-user-deactivate/live/start",
  requireAuth,
  ...
  async (req, res) => {
    try {
      ...
      const rawSelectedItems = Array.isArray(req.body?.selectedItems) ? req.body.selectedItems : [];
      const selectedItems = rawSelectedItems
        .map((item) => normalizeObusUserDeactivateSelectedItem(item))
        .filter(Boolean);

      if (selectedItems.length === 0) {
        return res.status(400).json({ ok: false, error: "En az bir kullanıcı seçmelisiniz." });
      }
      ...
      const itemMeta = selectedItems.map((item) => ({
        key: String(item?.value || "").trim(),
        label: buildObusUserDeactivateItemLabel(item)
      }));
      ...
      setImmediate(async () => {
        try {
          await deactivateObusUsersBySelection({
            selectedItems,
            cacheRows,
            onItemResult: (itemResult) => {
              pushObusLiveJobEvent(job, {
          ...
          finishObusLiveJob(job, err?.message || "Kullanıcı pasife alma işlemi tamamlanamadı.");
        }
      });
      return undefined;
    } catch (err) {
      return res.status(500).json({
        ok: false,
        error: `Canlı kullanıcı pasife alma başlatılamadı: ${err?.message || "Bilinmeyen hata"}`
      });
    }
  }
);
